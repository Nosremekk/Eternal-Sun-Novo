...
function BulbRenderer(_camera) constructor
{
    ...
    Update = function()
    {
        ...
        var _hdr = (hdr and _system.__hdrAvailable);
        ...
        if ((not _hdr) or (hdrBloomIterations != __oldHDRBloomIterations))
        {
            __FreeBloomSurfaces();
        }
        
        //Free up memory if the normal map state has changed
        if ((not normalMap) and __oldNormalMap)
        {
    ...
    DrawLitSurface = function(_surface, _x, _y, _width, _height, _textureFiltering = undefined, _alphaBlend = undefined)
    {
        ...
        if ((__lightSurface != undefined) and surface_exists(__lightSurface))
        {
            if (hdr and _system.__hdrAvailable
            and (hdrBloomIntensity > 0) and (hdrBloomIterations >= 1)
            and (__lightSurface != undefined) and surface_exists(__lightSurface))
            {
    ...
    GetTonemap = function()
    {
        return (hdr and _system.__hdrAvailable)? hdrTonemap : ldrTonemap;
    }
