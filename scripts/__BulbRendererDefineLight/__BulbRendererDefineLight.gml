...
function __BulbRendererDefineLight()
{
    ...
    {
        if ((__surfaceWidth <= 0) or (__surfaceHeight <= 0)) return undefined;
        
        var _surfaceWidth  = floor(__surfaceWidth);
        var _surfaceHeight = floor(__surfaceHeight);
        
        if ((__lightSurface != undefined) and ((surface_get_width(__lightSurface) != _surfaceWidth) or (surface_get_height(__lightSurface) != _surfaceHeight)))
        {
            surface_free(__lightSurface);
            __lightSurface = undefined;
        }
        
        if ((__lightSurface == undefined) or !surface_exists(__lightSurface))
        {
    ...
    GetLightValue = function(_worldX, _worldY)
    {
        ...
        {
            if ((_tonemap == BULB_TONEMAP_NONE) or (_tonemap == BULB_TONEMAP_CLAMP))
            {
                //Nothing else needed
            }
