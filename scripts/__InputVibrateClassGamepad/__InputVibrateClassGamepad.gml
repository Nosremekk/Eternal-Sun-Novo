...
function __InputVibrateClassGamepad(_device) constructor
{
    ...
    //device < 4 = XInput
    if ((INPUT_ON_WINDOWS and (__device < 4)) or (not INPUT_ON_WINDOWS))
    {
        __supported = true;
        ...
        else
        {
            ...
            if ((INPUT_ON_WINDOWS or INPUT_ON_SWITCH)
            and  ((_gamepadType == INPUT_GAMEPAD_TYPE_SWITCH)
              or (_gamepadType == INPUT_GAMEPAD_TYPE_JOYCON_LEFT)
              or (_gamepadType == INPUT_GAMEPAD_TYPE_JOYCON_RIGHT)))
            {
    ...
    {
        if (InputGameHasFocus() and __supported and __vibrating)
        {
