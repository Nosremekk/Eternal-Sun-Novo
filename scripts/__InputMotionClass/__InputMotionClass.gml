...
function __InputMotionClass(_device) constructor
{
    ...
    if (INPUT_ON_SWITCH or INPUT_ON_PS4 or INPUT_ON_PS5)
    {
    ...
    else if ((INPUT_ON_WINDOWS or INPUT_ON_LINUX) and InputGetSteamInfo(INPUT_STEAM_INFO.STEAMWORKS))
    {
        //Check to see if Steam thinks this device supports gyro & motion
        var _steamHandle = InputDeviceGetSteamHandle(__device);
        __hasData = is_numeric(_steamHandle) and is_struct(steam_input_get_motion_data(_steamHandle));
    }
    else
    {
        __hasData = false;
