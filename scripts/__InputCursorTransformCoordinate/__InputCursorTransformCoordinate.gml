...
__InputCursorTransformCoordinate(0, 0, INPUT_CURSOR_DEVICE_SPACE, INPUT_CURSOR_DEVICE_SPACE, undefined);

function __InputCursorTransformCoordinate(_x, _y, _inputSystem, _outputSystem, _camera = undefined)
{
    static _result = {
        x: 0,
        y: 0,
    ...
    if (_inputSystem != _outputSystem) //Only do MATHS if the output system is different
    {
        ...
        if ((_inputSystem == 0) or (_outputSystem == 0))
        {
            //If we have no defined camera then try to use view 0's camera
            if ((_camera == undefined) and view_enabled and view_visible[0]) _camera = view_camera[0];
