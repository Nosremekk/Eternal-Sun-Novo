...
function __BulbRendererDefineHDR()
{
    ...
    {
        if ((_width <= 0) or (_height <= 0)) return undefined;
        
        if ((__outputSurface != undefined) and ((surface_get_width(__outputSurface) != _width) or (surface_get_height(__outputSurface) != _height)))
        {
            surface_free(__outputSurface);
            __outputSurface = undefined;
        }
        
        if ((__outputSurface == undefined) or !surface_exists(__outputSurface))
        {
    ...
    {
        if ((__outputSurface != undefined) and surface_exists(__outputSurface))
        {
            surface_free(__outputSurface);
            __outputSurface = undefined;
        }
