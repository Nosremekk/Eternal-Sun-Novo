...
function __BulbRendererDefineNormal()
{
    ...
    GetNormalMapSurface = function()
    {
        ...
        if ((__normalSurface != undefined) and ((surface_get_width(__normalSurface) != __surfaceWidth) or (surface_get_height(__normalSurface) != __surfaceHeight)))
        {
            surface_free(__normalSurface);
            __normalSurface = undefined;
        }
        
        if ((__normalSurface == undefined) or !surface_exists(__normalSurface))
        {
    ...
    {
        if ((__normalSurface != undefined) and surface_exists(__normalSurface))
        {
            surface_free(__normalSurface);
            __normalSurface = undefined;
        }
